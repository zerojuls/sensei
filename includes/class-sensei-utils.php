...
class Sensei_Utils {
	...
	public static function sensei_delete_activities( $args = array() ) {
		...
		$comments = Sensei_Utils::sensei_check_for_activity( array(
	...
	public static function delete_all_user_activity( $user_id = 0 ) {
		...
		if ( $user_id ) {
			...
			$activities = Sensei_Utils::sensei_check_for_activity( array(
				'user_id' => $user_id,
			), true );
		...
		if ( $submitted && intval( $user_id ) > 0 ) {
			...
			foreach ( $submitted as $question_id => $answer ) {
				...
				switch ( $question_type ) {
					case 'multi-line': $answer = nl2br( $answer );
break;
					case 'single-line': break;
					case 'gap-fill': break;
					default: $answer = maybe_serialize( $answer );
break;
				}
				$args = array(
							'post_id' => $question_id,
	...
	public static function upload_file( $file = array() ) {
		 ...
		 * }
		 */
		$file_upload_args = apply_filters( 'sensei_file_upload_args', array(
			'test_form' => false,
		) );
	...
	public static function sensei_grade_question( $question_id = 0, $grade = 0, $user_id = 0 ) {
		...
		if ( intval( $question_id ) > 0 && intval( $user_id ) > 0 ) {
			...
			$user_answer_id = Sensei_Utils::sensei_get_activity_value( array(
				'post_id' => $question_id,
				'user_id' => $user_id,
	...
	public static function sensei_delete_quiz_answers( $quiz_id = 0, $user_id = 0 ) {
		...
		if ( intval( $quiz_id ) > 0 ) {
			$questions = Sensei_Utils::sensei_get_quiz_questions( $quiz_id );
			foreach ( $questions as $question ) {
				$delete_answers = Sensei_Utils::sensei_delete_activities( array(
					'post_id' => $question->ID,
					'user_id' => $user_id,
					'type' => 'sensei_user_answer',
				) );
	...
	public static function sensei_add_answer_notes( $question_id = 0, $user_id = 0, $notes = '' ) {
		...
		if ( intval( $question_id ) > 0 ) {
			...
			if ( ! empty( $notes ) ) {
				$user_lesson_id = Sensei_Utils::sensei_get_activity_value( array(
					'post_id' => $question_id,
					'user_id' => $user_id,
					'type' => 'sensei_user_answer',
					'field' => 'comment_ID',
	...
	public static function sensei_user_quiz_status_message( $lesson_id = 0, $user_id = 0, $is_lesson = false ) {
		...
		return apply_filters( 'sensei_user_quiz_status', array(
			'status' => $status,
			'box_class' => $box_class,
			'message' => $message,
			'extra' => $extra,
		), $lesson_id, $user_id, $is_lesson );
	}
	...
	public static function user_course_status( $course_id = 0, $user_id = 0 ) {
		...
		if ( $course_id ) {
			...
			$user_course_status = Sensei_Utils::sensei_check_for_activity( array(
				'post_id' => $course_id,
				'user_id' => $user_id,
				'type' => 'sensei_course_status',
			), true );
	...
	public static function user_lesson_status( $lesson_id = 0, $user_id = 0 ) {
		...
		if ( $lesson_id > 0 && $user_id > 0 ) {
			...
			$user_lesson_status = Sensei_Utils::sensei_check_for_activity( array(
				'post_id' => $lesson_id,
				'user_id' => $user_id,
				'type' => 'sensei_lesson_status',
			), true );
	...
	public static function get_user_data( $data_key, $post_id, $user_id = 0 ) {
		...
		// check for existing Sensei status on this post type if not, create it and get the  activity ID.
		$status_function = 'user_' . $post_type . '_status';
		$sensei_user_status = self::$status_function( $post_id ,$user_id  );
		if ( ! isset( $sensei_user_status->comment_ID ) ) {
			return false;
	 ...
	 * @since 1.8.0
	 *
	 * @param string $selected_value Select Value.
	 * @param array  $options {.
	 ...
	 * }.
	 * @param bool   $enable_none_option Enable none.
	 *
	...
	public static function generate_drop_down( $selected_value, $options = array(), $attributes = array(), $enable_none_option = true ) {
		...
		foreach ( $attributes as $attribute => $value ) {
			...
			$combined_attributes[] = esc_attr( $attribute ) . '="' . esc_attr( $value ) . '"';

		}// End foreach().

		// create the select element.
		$drop_down_element .= '<select ' . implode( ' ', $combined_attributes ) . ' >' . "\n";
	 ...
	 * Is a plugin preset?
	 *
	 * @param string $plugin_class_to_look_for The plugin class.
	 * @param string $plugin_registered_path The plugin registered path.
	 *
	 * @return bool
	 */
	public static function is_plugin_present_and_activated( $plugin_class_to_look_for, $plugin_registered_path ) {
		...
		$plugin_present_and_activated = in_array( $plugin_registered_path, $active_plugins, true ) || array_key_exists( $plugin_registered_path, $active_plugins );
	...
	/**
	 * Get a Setting as a flog (cast it to bool)
	 *
	 * @param string      $setting_name Setting Name.
	 * @param null|string $filter_to_apply Filter to apply.
	 * @return bool
	 */
